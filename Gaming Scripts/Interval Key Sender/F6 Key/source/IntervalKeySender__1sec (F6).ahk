...
;; The script is turned off by default and has to be turned on by pressing the (Home) key.

;; User Data

Menu, Tray, Tip, Interval Key Sender`n1 Second Version`n`nHit (Home) key to send the (F6) key every second`nHit (Home) key again to turn it off

{
	global SPAM_KEY := "F6" ;; Set the key combo for quick saving
